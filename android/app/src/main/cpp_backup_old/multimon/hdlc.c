...
static void ax25_disp_packet(struct demod_state *s, unsigned char *bp, unsigned int len)
{
	...
	// Debug: dump first 20 bytes of packet in hex (single line)
	char hexbuf[128];
	int pos = 0;
	for (i = 0; i < len && i < 20 && pos < 100; i++) {
		pos += snprintf(hexbuf + pos, sizeof(hexbuf) - pos, "%02X ", bp[i]);
	}
	verbprintf(1, "[HDLC] Packet hex (%u bytes): %s\n", len, hexbuf);
	
#if 1
	if (!check_crc_ccitt(bp, len)) {
		verbprintf(1, "[HDLC] CRC check failed for %u byte packet\n", len);
		return;
	}
	verbprintf(1, "[HDLC] CRC check passed for %u byte packet\n", len);
#endif
	len -= 2;
        if (bp[1] & 1) {
                /*
...
void hdlc_rxbit(struct demod_state *s, int bit)
{
	...
	if ((s->l2.hdlc.rxbitstream & 0xff) == 0x7e) {
		flag_count++;
		// Log every 10th flag detection to show activity
		if (flag_count % 10 == 1) {
			verbprintf(1, "[HDLC] Flag 0x7E detected (#%d after %d bits)\n", flag_count, bit_count);
		}
		if (s->l2.hdlc.rxstate && (s->l2.hdlc.rxptr - s->l2.hdlc.rxbuf) > 2) {
			verbprintf(1, "[HDLC] Frame complete, %ld bytes\n", (long)(s->l2.hdlc.rxptr - s->l2.hdlc.rxbuf));
